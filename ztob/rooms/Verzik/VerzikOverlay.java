...
public class VerzikOverlay extends RoomOverlay
{
	...
	public Dimension render(Graphics2D graphics)
	{
		if (verzik.isVerzikActive() && config.verzikMelee())
		{
			...
			if (lp != null)
			{
				Polygon tilePoly = Perspective.getCanvasTileAreaPoly(client, lp, size, false);
				if (tilePoly != null)
				{
		...
		if (verzik.isVerzikActive() && (config.verzikNyloExplodeRange() || config.verzikNyloAggroWarning()))
		{
			verzik.getVerzikAggros().forEach(k ->
			{
				...
				if (config.verzikNyloExplodeRange())
				{
					int size = 1;
					int thick_size = 1;
		...
		if (verzik.isVerzikActive() && config.verzikReds())
		{
			verzik.getVerzikReds().forEach((k, v) ->
			{
				...
				if (k.getName() != null && k.getHealth() > 0)
				{
					v_health = k.getHealth();
			...
			for (NPC npc : reds)
			{
				if (npc.getName() != null && npc.getHealth() > 0 && npc.getHealthRatio() < 100)
				{
					Pair<Integer, Integer> newVal = new MutablePair<>(npc.getHealthRatio(), npc.getHealth());
